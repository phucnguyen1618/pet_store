        ...
        if (result.invoiceNumber.isNotEmpty) {
          ...
          if (response.href.isNotEmpty) {
            ...
            if (result.href.isNotEmpty) {
              ...
              List<OrderDetail> orderDetails = cartList.map((e) => OrderDetail(e.product.idProduct, e.quantity, e.product.getPrice() * e.quantity)).toList();
              String idOrder = const Uuid().v4();
              Order order = Order(idOrder, idUser, orderDetails, DateTime.now(), totalProductPrice, 1);
